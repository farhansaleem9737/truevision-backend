...
// ── Call-log helpers (best-effort; never block the signalling path) ─────────
const logCallStart = (data) => Call.create(data).catch(() => {});
const updateCall   = (callId, patch) =>
  Call.updateOne({ callId }, { $set: patch }).catch(() => {});
...
const initSocket = (httpServer) => {
  io = new Server(httpServer, {
    // TODO (production): restrict Socket.IO CORS `origin: '*'` to the deployed
    // app origin(s) before release. Left open in development for Expo Go / LAN
    // devices. Mitigated today because auth is a handshake token, not a cookie.
  ...
  io.on('connection', async (socket) => {
    ...
    // Pure relay (no presence persistence needed — it's ephemeral and the
    // client also auto-clears on a timeout, like typing).
    socket.on('recording', ({ chatId }) => {
      if (!chatId) return;
      socket.to(`chat:${chatId}`).emit('recording', { chatId, userId: uid, username: socket.userData.username });
    });

    socket.on('stopRecording', ({ chatId }) => {
      if (!chatId) return;
      socket.to(`chat:${chatId}`).emit('stopRecording', { chatId, userId: uid });
    });

    // ── Call signalling (WebRTC) ────────────────────────────────────────
    // The server is a thin relay + gatekeeper. Media is peer-to-peer; only the
    // SDP offer/answer and ICE candidates pass through here, routed to the
    // peer's personal room. `callId` is generated by the caller and echoed on
    // every message so both sides + the Call log share one identifier.

    // Caller → callee: ring + deliver the SDP offer.
    socket.on('call:invite', async ({ toUserId, callId, mode, sdp }) => {
      try {
        if (!toUserId || !callId || String(toUserId) === uid) return;

        // A block in either direction silently fails the call.
        if (await privacy.isBlockedBetween(uid, toUserId)) {
          socket.emit('call:declined', { callId, reason: 'unavailable' });
          return;
        }

        const kind = mode === 'video' ? 'video' : 'audio';
        logCallStart({ callId, caller: uid, callee: toUserId, mode: kind, status: 'ringing' });

        // Callee offline → immediate "unavailable" + log as missed.
        if (!presence.isOnline(String(toUserId))) {
          socket.emit('call:unavailable', { callId, reason: 'offline' });
          updateCall(callId, { status: 'missed', endedAt: new Date() });
          return;
        }
        ...
        emitToUser(toUserId, 'call:incoming', {
          ...
          from: {
            ...
            profileImage: socket.userData.profileImage,
            isVerified:   socket.userData.isVerified,
          },
        });
      } catch (err) {
    ...
    // Callee → caller: accept + deliver the SDP answer.
    socket.on('call:accept', ({ toUserId, callId, sdp }) => {
      if (!toUserId || !callId) return;
      emitToUser(toUserId, 'call:accepted', { callId, sdp });
      updateCall(callId, { status: 'answered', answeredAt: new Date() });
    });

    // Callee → caller: decline.
    socket.on('call:decline', ({ toUserId, callId }) => {
      if (!toUserId || !callId) return;
      emitToUser(toUserId, 'call:declined', { callId, reason: 'declined' });
      updateCall(callId, { status: 'declined', endedAt: new Date() });
    });

    // Callee already busy on another call.
    socket.on('call:busy', ({ toUserId, callId }) => {
      if (!toUserId || !callId) return;
      emitToUser(toUserId, 'call:busy', { callId });
      updateCall(callId, { status: 'busy', endedAt: new Date() });
    });

    // Caller cancels before the callee answered → missed on the callee side.
    socket.on('call:cancel', ({ toUserId, callId }) => {
      if (!toUserId || !callId) return;
      emitToUser(toUserId, 'call:cancelled', { callId });
      updateCall(callId, { status: 'missed', endedAt: new Date() });
    });
    ...
    socket.on('call:ice', ({ toUserId, callId, candidate }) => {
      ...
      emitToUser(toUserId, 'call:ice', { callId, candidate });
    });

    // Either side hangs up an active call.
    socket.on('call:end', ({ toUserId, callId, durationSec }) => {
      if (!toUserId || !callId) return;
      emitToUser(toUserId, 'call:ended', { callId });
      updateCall(callId, {
